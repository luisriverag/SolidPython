...
from solid.utils import fillet_2d, is_scad, offset_points
...
other_test_cases = [
    ...
    (
        ...
        offset_points,
        [tri, 2, True],
        "[Point2(2.00, 2.00), Point2(5.17, 2.00), Point2(2.00, 5.17)]",
    ),
    (
        "offset_points_outside",
        offset_points,
        [tri, 2, False],
        "[Point2(-2.00, -2.00), Point2(14.83, -2.00), Point2(-2.00, 14.83)]",
    ),
]
...
class TestSPUtils(DiffOutput):
    ...
    def assertEqualOpenScadObject(
        self, expected: str, actual: Union[OpenSCADObject, str]
    ):
    ...
    def test_fillet_2d_add(self):
        ...
        actual = fillet_2d(
            three_points, orig_poly=p, fillet_rad=2, remove_material=False
        )
    ...
    def test_euclidify_non_mutating(self):
        ...
        next_tri = euclidify(base_tri, Point2)  # noqa: F841
        expected = 3
        actual = len(base_tri)
    ...
    def test_offset_points_open(self):
        ...
        expected = [
            [0.0, 1.0],
            [7.585786437626904, 1.0],
            [-0.7071067811865479, 9.292893218813452],
        ]
        self.assertEqual(expected, actual)
...
def generate_scad_test(func, args, expected):
    def test_scad(self):
        scad_obj = func(*args)
...
def generate_no_scad_test(func, args, expected):
...
def create_tests():
    for test_tuple in scad_test_cases:
        ...
        test = generate_scad_test(func, args, expected)
        setattr(TestSPUtils, test_name, test)

    for test_tuple in other_test_cases:
        test_name, func, args, expected = read_test_tuple(test_tuple)
        test = generate_no_scad_test(func, args, expected)
        setattr(TestSPUtils, test_name, test)
