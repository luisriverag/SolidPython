...
from math import ceil, sqrt, atan2, degrees

from solid import union, cube, translate, rotate, square, circle, polygon
...
from solid import OpenSCADObject, Vec3
...
# = TYPING =
# ==========
from typing import Any, Union, Sequence, Optional, Callable, cast  # noqa
...
EucOrTuple = Union[Point3, Vector3, Tuple2, Tuple3]
...
# ==============
def grid_plane(
    grid_unit: int = 12, count: int = 10, line_weight: float = 0.1, plane: str = "xz"
) -> OpenSCADObject:
    ...
    for i in range(int(-count / 2), int(count / 2 + 1)):
        if "xz" in plane:
            # xz-plane
            h = up(i * grid_unit)(cube([ls, line_weight, line_weight], center=True))
            v = right(i * grid_unit)(cube([line_weight, line_weight, ls], center=True))
            t.add([h, v])

        # xy plane
        if "xy" in plane:
            h = forward(i * grid_unit)(
                cube([ls, line_weight, line_weight], center=True)
            )
            v = right(i * grid_unit)(cube([line_weight, ls, line_weight], center=True))
            t.add([h, v])

        # yz plane
        if "yz" in plane:
            h = up(i * grid_unit)(cube([line_weight, ls, line_weight], center=True))
            v = forward(i * grid_unit)(
                cube([line_weight, line_weight, ls], center=True)
            )
...
def distribute_in_grid(
    objects: Sequence[OpenSCADObject],
    max_bounding_box: tuple[float, float],
    rows_and_cols: tuple[int, int] = None,
) -> OpenSCADObject:
...
# =======
def arc(
    rad: float, start_degrees: float, end_degrees: float, segments: int = None
) -> OpenSCADObject:
...
def arc_inverted(
    ...
    top_half_square = translate((-(wide - rad), 0, 0))(
        square([wide, high], center=False)
    )
    bottom_half_square = translate((-(wide - rad), -high, 0))(
        square([wide, high], center=False)
    )
...
) -> tuple[OpenSCADObject, BoundingBox, OpenSCADObject, BoundingBox]:
...
def _table_string(
    field_names: Sequence[str], rows: Sequence[Sequence[float]], csv: bool = False
) -> str:
...
def label(
    ...
    for idx, line in enumerate(lines):
        t = text(
            text=line, halign=halign, valign=valign, font=font, spacing=spacing
...
# ==================
def euclidify(
    an_obj: EucOrTuple, intended_class: type = Vector3
) -> Union[Point23, Vector23, list[Union[Point23, Vector23]]]:
    """
...
def centroid(points: Sequence[PointVec23]) -> PointVec23:
    if not points:
        raise ValueError("centroid(): argument `points` is empty")
    first = points[0]
...
# ========================================
def draw_segment(
    euc_line: Union[Vector3, Line3] = None,
    endless: bool = False,
    arrow_rad: float = 7,
    vec_color: Union[str, Tuple3] = None,
) -> OpenSCADObject:
...
def offset_points(
    points: Sequence[Point23], offset: float, internal: bool = True, closed=True
) -> list[Point2]:
    """
    ...
    perp_vecs = list(
        (perpendicular_vector(v, direction=direction, length=offset) for v in vecs)
    )
...
def offset_point(
    a: Point2, b: Point2, c: Point2, offset: float, direction: DirectionLR = LEFT_DIR
) -> Point2:
...
def pairwise_zip(ls: Sequence) -> zip:  # type:ignore
...
def perpendicular_vector(
    v: Vector2, direction: DirectionLR = RIGHT_DIR, length: float = None
) -> Vector2:
...
def fillet_2d(
    three_point_sets: Sequence[tuple[Point23, Point23, Point23]],
    ...
    for three_points in three_point_sets:
        ...
        arc_obj = translate(center.as_arr())(
            arc_inverted(
                rad=fillet_rad, start_degrees=start_degrees, end_degrees=end_degrees
            )
        )
...
def _widen_angle_for_fillet(
    start_degrees: float, end_degrees: float
) -> tuple[float, float]:
    # Fix start/end degrees as needed; find a way to make an acute angle
    if end_degrees < start_degrees:
        end_degrees += 360
...
# ==============
def path_2d(
    points: Sequence[Point23], width: float = 1, closed: bool = False
) -> list[Point2]:
    """
    ...
    p_b = list(
        reversed(offset_points(points, offset=width / 2, internal=False, closed=closed))
    )
    return p_a + p_b
...
def lerp(
    val: float, min_in: float, max_in: float, min_out: float, max_out: float
) -> float:
...
from solid.extrude_along_path import extrude_along_path  # noqa
